...
class MultiStreamSharedOR(nn.Module):
    '''
    Multi-Stream model with shared weights for ordinal regression.
    '''

    def __init__(self, model_img, model_flow, num_m, n_classes):
        super(MultiStreamSharedOR, self).__init__()
        ...
        self.thresholds = nn.Parameter(torch.tensor(range(10)).float(), requires_grad=True)
